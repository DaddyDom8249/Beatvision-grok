...
function StoryboardPage() {
  ...
  function addScene() {
    ...
    const scene: StoryboardScene = {
      ...
      characterIds: lockedWorld.characters[0]
        ? [lockedWorld.characters[0].id]
        : [],
      camera: {
        scale: "medium",
        move: "static",
  ...
  async function saveAndLock() {
    ...
    const payload = { storyboard: locked, lockedAt: locked.lockedAt };
    ...
    setSavedMsg("Storyboard locked and saved to database.");
  }

  async function saveDraft() {
    if (!storyboard) return;
    const payload = { storyboard, lockedAt: null };
    sessionStorage.setItem("bv-storyboard", JSON.stringify(payload));
    const id = sessionStorage.getItem("bv-project-id");
    if (id) {
      try {
    ...
    setSavedMsg("Draft saved to database.");
  }
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 mx-auto w-full max-w-5xl px-4 py-8 space-y-8">
        <div className="rounded-xl border border-[var(--bv-border)] bg-[var(--bv-surface)] p-4 text-sm text-[var(--bv-muted)] leading-relaxed">
          ...
          {isRealDuration
            ? draft.audioName
              ? ` from measured audio (${draft.audioName}). Sections are proportional to the real song length.`
              : " from measured audio duration. Sections are proportional to the real song length."
            : draft.audioName
              ? ` (filename on file: ${draft.audioName}, but duration not measured yet). Upload audio on the home step to lock real timing.`
              : ". No measured duration — estimated from lyrics length. Upload audio on the home step for a real master timeline."}
                  ...
                  >
                    {timeline.sections.map((s) => (
                      <option key={s.id} value={s.id}>
                        {s.label ?? s.kind} ({s.startSec.toFixed(1)}–
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl border border-[var(--bv-border)] bg-[var(--bv-surface)] p-5 space-y-3 sticky top-4">
              ...
              <p className="text-[11px] text-[var(--bv-muted)] leading-relaxed pt-2 border-t border-[var(--bv-border)]">
                No generation runs here. Media and final video are later phases.
                Validation prevents silent media recycling.
              </p>
            </div>
          </aside>
        </div>
